...
app.whenReady().then(async () => {
  for (const [width, height] of SIZES) {
    ...
    await waitFor(win, LAUNCH, { what: 'the Launch Clone Hero button' })
    await sleep(600)
    ...
    await waitFor(win, `document.querySelector('.topbar-note')`, { what: "the top bar's note" })
    await sleep(200)
